	...
	const std::shared_ptr<Shader> &renderImageShader)
{
	...
	_lightScatteringShader->setShaderTexture("pgeGBufferPosition", getRenderScene()->_gBuffer.getTextureID(GBuffer::_positionAndEmissive), GL_TEXTURE_2D);
}
...
void SceneObjectLightScattering::postRender() {
	...
	_lightScatteringShader->setUniformv2f("pgeSizeInv", sizeInv);

	_lightScatteringShader->setUniformv3f("pgeColor", _lightSourceColor * std::max(0.0f, (getScene()->_logicCamera._rotation * Vec3f(0.0f, 0.0f, -1.0f)).dot((_lightSourcePosition - getScene()->_logicCamera._position).normalized())));

	_lightScatteringShader->setUniformv4f("pgeLightProjectedPosition", getScene()->_logicCamera.getProjectionViewMatrix() * Vec4f(_lightSourcePosition.x, _lightSourcePosition.y, _lightSourcePosition.z, 1.0f));
	_lightScatteringShader->setUniformv3f("pgeLightViewPosition", getScene()->_logicCamera.getViewMatrix() * _lightSourcePosition);
