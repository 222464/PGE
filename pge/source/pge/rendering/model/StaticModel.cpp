    ...
    while (!fromFile.eof()) {
        ...
        if (header == "v") {
            ...
            for (size_t i = 1; i < v.size() - 1; i++) {
                                meshes.back().mesh->indices.push_back(static_cast<staticMeshIndexType>(firstIndex));

                                for (int j = 0; j < 2; j++) {
                                        int d = i + j;

                                        // Search for index set 1
                                        std::unordered_map<StaticMeshIndexSet, size_t, StaticMeshIndexSet>::iterator it = indexToVertex.find(v[d]);

                                        if (it == indexToVertex.end()) {
                                                // Vertex attributes do not exist, create them

                                                // File indicies start at 1, so convert
                                                size_t vertIndex = v[d].vi - 1;
                                                size_t texCoordIndex = v[d].ti - 1;
                                                size_t normalIndex = v[d].ni - 1;
